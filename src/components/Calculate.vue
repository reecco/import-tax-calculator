...
onMounted(() => {
  const currencyStorage: CurrencyStorage | null = JSON.parse(localStorage.getItem("currency") as any);

  if (!currencyStorage || verifyDate(currencyStorage.timestamp)) {
    api
      .then((res) => res.json())
      .then((res) => {
  ...
  currency.value.dolar = currencyStorage.dolar;
});
...
<template>
  ...
  >
    <Form 
      class="my-10 flex flex-col justify-center items-center h-[32rem] w-[28rem] bg-[#f3f3f3] border-[#dedddd] border-[1px] border-solid rounded-[10px]"
    >
      <div
      ...
      >
        ...
        >
          <label 
            class="" 
            for="buy"
          >Valor da compra em US$
          </label>
          <input 
            class="w-72 h-10 rounded-[6px] pl-2 mt-1" 
            id="buy" 
            placeholder="0,00" 
            type="number" 
            v-model="price" 
        ...
        >
          ...
          >
            <input 
              ...
              id="dispatchFee" 
              v-model="dispatchFee" 
              @input="handleCheckDispatch" 
              :disabled="(price as number) > 0 ? false : true"
            >
            <label 
              class="pl-2 w-[250px] h-[50px]"
              for="dispatchFee"
            >Incluir taxa de despacho dos Correios (R$ 15,00)
            </label>
          </div>
          <div
            class="mt-5 flex items-center"
          >
            <input 
              ...
              id="is-prc" 
              v-model="isPRC" 
              @input="handleCheckPRC" 
              :disabled="(price as number) > 0 ? false : true"
            >
            <label 
              class="pl-2 w-[250px] h-[50px]"
              for="is-prc"
            >Empresa participante do Programa Remessa Conforme
            </label>
          </div>
        </div>
        <div 
          class="flex flex-col items-center mt-8 w-72"
        >
          <label 
            class="text-center" 
            for="amount-usd"
          ...
          <div
            class="flex flex-row justify-around w-full mt-4"
          >
            <h3 
              class="bg-[#fefefe] p-2 rounded-[5px] border-[#dedddd] border-[1px] border-solid" 
              id="amount-usd" 
              v-if="resultQuotation"
            >US$ {{ resultQuotation }}
            </h3>
            <h3 
              class="bg-[#fefefe] p-2 rounded-[5px] border-[#dedddd] border-[1px] border-solid" 
              id="amount-brl" 
              v-if="resultQuotation"
            >R$ {{ (parseInt(resultQuotation) * currency.dolar).toFixed(2).replace(".", ",") }}
            </h3>
          </div>
          <p 
            class="mt-8 bg-[#fefefe] w-[100%] text-center rounded-[5px]" 
            v-if="message"
          >{{ message }}
          </p>
        </div>
